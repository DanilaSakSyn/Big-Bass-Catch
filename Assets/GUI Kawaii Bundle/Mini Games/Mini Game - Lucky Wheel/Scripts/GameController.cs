...
namespace FortuneWheel
{
    public class GameController : MonoBehaviour
    {
        ...
        public Text totalGoldText;           // Pop-up text with cost or rewarded coins amount
        ...
        private int  totalEnergy, totalLife, totalGem1, totalGem2, totalGem3, totalGem4, totalMoney;                                          // Started coins amount. In your project it can be set up from PlayerProgress, DataController or from PlayerPrefs 
        ...
        private void Awake()
        {
            ...
            //UI declerations
            previousGold = Wallet.Instance.GetBalance();
            totalGoldText.text = Wallet.Instance.GetBalance().ToString();
        ...
        public void TurnWheel()
        {
            ...
            if (Wallet.Instance.GetBalance() >= turnCost) // If player has enough gold to turn the wheel
            {
                ClaimTurnCost();
        ...
        {
            previousGold = Wallet.Instance.GetBalance();   //Set prev value for flipping uı animation
            Wallet.Instance.SpendCoins(turnCost);
        ...
        public void ClaimReward()
        {
            ...
            switch (PiecesOfWheel[rewardIndex].rewardCategory)
            {
                case RewardEnum.Gold:
                    {
                        Wallet.Instance.AddCoins(PiecesOfWheel[rewardIndex].rewardAmount * rewardMultiplier);
                    }
                    break;
        ...
        private IEnumerator UpdateRewardAmount()
        {
            ...
            {
                totalGoldText.text = Mathf.Floor(Mathf.Lerp(previousGold, Wallet.Instance.GetBalance(), (elapsedTime / seconds))).ToString();
